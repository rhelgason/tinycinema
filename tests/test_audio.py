...
def test_resume_relaunches_at_the_paused_position(monkeypatch):
    monkeypatch.setattr("tinycinema.audio.ffplay.ffplay_path", lambda: "/bin/ffplay")
    sink = FFplaySink("clip.mp4")
    sink._paused = True
    sink._paused_position = 5.0
    started = []
    monkeypatch.setattr(sink, "start", lambda pos=0.0: started.append(pos))
    sink.resume()
    assert started == [5.0]
    assert not sink._paused
